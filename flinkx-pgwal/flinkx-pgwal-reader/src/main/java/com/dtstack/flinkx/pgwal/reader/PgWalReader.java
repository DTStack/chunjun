...
public class PgWalReader extends DataReader {
    ...
    protected PgWalReader(DataTransferConfig config, StreamExecutionEnvironment env) {
        super(config, env);
        ...
        statusInterval = readerConfig.getIntVal(PgWalConfigKeys.KEY_STATUS_INTERVAL, 10000);
        lsn = readerConfig.getStringVal(PgWalConfigKeys.KEY_LSN);
        slotName = readerConfig.getStringVal(PgWalConfigKeys.KEY_SLOT_NAME);
        allowCreateSlot = readerConfig.getBooleanVal(PgWalConfigKeys.KEY_ALLOW_CREATE_SLOT, true);
        temporary = readerConfig.getBooleanVal(PgWalConfigKeys.KEY_TEMPORARY, true);
    }
