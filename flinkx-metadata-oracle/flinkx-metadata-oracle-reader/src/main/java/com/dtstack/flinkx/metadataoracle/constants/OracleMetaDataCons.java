...
public class OracleMetaDataCons extends MetaDataCons {
    ...
    public static final String SQL_QUERY_COLUMN = "SELECT COLUMNS.COLUMN_NAME, COLUMNS.DATA_TYPE, COMMENTS.COMMENTS  FROM DBA_TAB_COLUMNS COLUMNS JOIN DBA_COL_COMMENTS COMMENTS \n" +
            "ON COLUMNS.OWNER = COMMENTS.OWNER AND COLUMNS.TABLE_NAME = COMMENTS.TABLE_NAME AND COLUMNS.COLUMN_NAME = COMMENTS.COLUMN_NAME\n" +
            "WHERE COLUMNS.OWNER = %s AND COLUMNS.TABLE_NAME = %s";
    public static final String SQL_QUERY_TABLE_PROPERTIES = "SELECT TABLES.NUM_ROWS*TABLES.AVG_ROW_LEN AS TOTALSIZE, COMMENTS.COMMENTS, COMMENTS.TABLE_TYPE, OBJS.CREATED, TABLES.NUM_ROWS \n" +
    ...
    // 只同步用户库
    public static final String SQL_SHOW_DATABASES = "SELECT USERNAME FROM USER_USERS";
