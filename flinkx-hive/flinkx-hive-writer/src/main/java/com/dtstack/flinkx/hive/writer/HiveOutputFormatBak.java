 ...
 * @author sishu.yss
 */
public abstract class HiveOutputFormatBak extends RichOutputFormat {

    private static final long serialVersionUID = -6012196822223887479L;

    private static Logger logger = LoggerFactory.getLogger(HiveOutputFormatBak.class);
...
//    protected Configuration configuration;
    ...
    private Map<String, HiveOutputFormatBak> outputFormats = Maps.newConcurrentMap();
...
//        fs = FileSystem.get(conf);
    ...
    private void emit(Map event) {
        ...
        try {
            ...
            HiveOutputFormatBak format = getHdfsOutputFormat(tablePath, event);
...
//                dirtyDataManager.writeData(event, e);
    ...
    public HiveOutputFormatBak getHdfsOutputFormat(String tablePath, Map event) throws Exception {
        ...
        HiveOutputFormatBak outputFormat = outputFormats.get(hiveTablePath);
        if (outputFormat == null) {
            ...
            outputFormat = (HiveOutputFormatBak) builder.finish();
...
//            outputFormats.put(hiveTablePath, hdfsOutputFormat);
    ...
    private TableInfo checkCreateTable(String tablePath, Map event) throws Exception {
        try {
            if (!tableCache.containsKey(tablePath)) {
                synchronized (HiveOutputFormatBak.class) {
                    if (!tableCache.containsKey(tablePath)) {
    ...
    private void closeHdfsOutputFormats() {
        Iterator<Map.Entry<String, HiveOutputFormatBak>> entryIterator = outputFormats.entrySet().iterator();
        while (entryIterator.hasNext()) {
            try {
                Map.Entry<String, HiveOutputFormatBak> entry = entryIterator.next();
...
//    @Override
    protected void hiveOpenInternal(int taskNumber, int numTasks) throws IOException {
...
//        conf = HdfsUtil.getHadoopConfig(hadoopConfig, defaultFS);
        ...
        hiveBeforeOpenInternal();
        open();
    }

//    @Override
    protected void hiveBeforeOpenInternal(){
        if(taskNumber > 0){
            return;
        }
    ...
    private void flushHiveBlock() throws IOException {
...
//    @Override
    public void hiveTryCleanupOnError() throws Exception {
...
//    @Override
    protected boolean hiveNeedWaitAfterCloseInternal() {
        return true;
    }
