...
public abstract class CommonExecLookupJoin extends ExecNodeBase<RowData>
    ...
    public Transformation<RowData> translateToPlanInternal(PlannerBase planner) {
        ...
        if (temporalTable instanceof TableSourceTable) {
            List<Integer> lookupKeyIndicesInOrder =
                    lookupKeys.keySet().stream().sorted().collect(Collectors.toList());
            int[][] keys = new int[lookupKeyIndicesInOrder.size()][];
            lookupKeyIndicesInOrder.stream()
                    .forEach(index -> keys[index] = new int[] {lookupKeyIndicesInOrder.get(index)});
            LookupRuntimeProviderContext providerContext = new LookupRuntimeProviderContext(keys);
            LookupTableSource.LookupRuntimeProvider provider =
                    ((LookupTableSource) temporalTable).getLookupRuntimeProvider(providerContext);
            if (provider instanceof ParallelismProvider) {
                Optional<Integer> parallelismOptional =
                        ((ParallelismProvider) provider).getParallelism();
                if (parallelismOptional.isPresent()) {
                    int parallelismPassedIn = parallelismOptional.get().intValue();
                    if (parallelismPassedIn <= 0) {
                        throw new TableException(
                                "Table: configured lookup parallelism: "
                                        + "$parallelismPassedIn should not be less than zero or equal to zero");
                    }
                    parallelism = parallelismPassedIn;
                }
            }
        }
