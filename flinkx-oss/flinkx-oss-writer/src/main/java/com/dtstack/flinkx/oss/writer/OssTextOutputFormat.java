...
import com.dtstack.flinkx.oss.ECompressType;
...
/**
 * The builder class of OssOutputFormat writing text files
 *
 * @author wangyulei
 * @date 2021-06-30
 */
public class OssTextOutputFormat extends BaseOssOutputFormat {
    ...
    protected void nextBlock(){
        ...
        try {
            String currentBlockTmpPath = null;
            if (tmpPath.endsWith("/")) {
                currentBlockTmpPath = tmpPath + currentBlockFileName;
            } else {
    ...
    public void writeSingleRecordToFile(Row row) throws WriteRecordException {
        ...
        try {
            ...
            LOG.error(e.getMessage(), e);
            throw new WriteRecordException(String.format("数据写入oss异常，row:{%s}", row), e);
        }
    }

    private void appendDataToString(StringBuilder sb, Object column, ColumnType columnType) {
        if (column == null) {
            sb.append(OssUtil.NULL_VALUE);
            return;
        }
