  ...
  const buildChildren = children => {
    return children.map(c => {
      if (c.type === "dir") {
        ...
        return (
          <Link to={`/documents/${c.data.id}`} className={`w-full px-[20px] rounded-sm cursor-pointer hover:bg-gray-100 h-[48px] flex items-center ${c.data.id === currentPage ? "active" : null}`}>
            {c.name}
          </Link>
        )
      }
      ...
      <aside className="md:inline-block hidden shadow-sm h-screen overflow-y-auto px-[12px] py-[20px]">
        {children.map(item => {
          return item.type === "file" ? (
            <Link to={`/documents/${item.data.id}`} className={`w-full px-[20px] rounded-sm cursor-pointer hover:bg-gray-100 h-[48px] flex items-center ${item.data.id === currentPage ? "active" : null}`}>
              {item.name}
            </Link>
          ) : (
            <Accordion iconPosition="right">
