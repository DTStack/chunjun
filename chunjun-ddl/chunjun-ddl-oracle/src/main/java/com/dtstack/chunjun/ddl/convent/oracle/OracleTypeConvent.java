...
import com.dtstack.chunjun.ddl.convent.oracle.type.OracleColumnTypeDescUtil;
import com.dtstack.chunjun.ddl.convent.oracle.type.OracleType;
...
public class OracleTypeConvent implements ColumnTypeConvent {
    ...
    private static void initMysqlToCommonType() {
        ...
        Oracle_TO_COMMON_TYPE.put(OracleType.DATE.getSourceName(), CommonColumnTypeEnum.DATE);
        Oracle_TO_COMMON_TYPE.put(
    ...
    private static void initAllColumnType() {
        Arrays.stream(OracleType.values()).forEach(i -> ORACLE_COLUMN_TYPE.add(i.name()));
    }
}
