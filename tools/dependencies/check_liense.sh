...
if [ -d "/tmp/chunjun-dependencies" ]; then
  rm -rf /tmp/chunjun-dependencies/*
fi

./mvnw clean package -Dmaven.test.skip dependency:copy-dependencies -DincludeScope=runtime -DoutputDirectory=/tmp/chunjun-dependencies

ls /tmp/chunjun-depnendencies | sort > all-dependencies.txt

echo "---------- licenses check start -------------"

# licenses
echo '=== Self modules: ' && ./mvnw --quiet -Dexec.executable='echo' -Dexec.args='${project.artifactId}-${project.version}.jar' exec:exec | tee self-modules.txt
