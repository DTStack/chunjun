...
public abstract class AbstractYarnClusterDescriptor implements ClusterDescriptor<ApplicationId> {
    ...
    private PackagedProgram buildProgram(String monitorUrl,ClusterSpecification clusterSpecification) throws Exception{
        ...
        if (null != clusterSpecification.getSpSetting()) {
    ...
    private void dealPluginByLoadMode(JobGraph jobGraph) throws Exception {
        String pluginLoadMode = flinkConfiguration.getString("pluginLoadMode", "shipfile");
        if (StringUtils.equalsIgnoreCase(pluginLoadMode, "classpath")) {
