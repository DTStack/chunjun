...
public class YarnClusterDescriptor implements ClusterDescriptor<ApplicationId> {
	...
	private Optional<List<File>> decodeDirsToShipToCluster(final Configuration configuration) {
		...
		final List<File> files = ConfigUtils.decodeListFromConfig(configuration, YarnConfigOptions.SHIP_FILES, File::new);
	...
	private static YarnConfigOptions.UserJarInclusion getUserJarInclusionMode(org.apache.flink.configuration.Configuration config) {
		return config.getOptional(YarnConfigOptions.CLASSPATH_INCLUDE_USER_JAR).orElse(YarnConfigOptions.UserJarInclusion.DISABLED);//.getEnum(YarnConfigOptions.UserJarInclusion.class, YarnConfigOptions.CLASSPATH_INCLUDE_USER_JAR);
	}
	 ...
	 * {@link YarnApplicationFileUploader#(Collection, String)}
		...
		try {
			...
			try (final FileSystem fs = FileSystem.get(yarnConfiguration)) {
				...
				Utils.deleteApplicationFiles(Collections.singletonMap(
						...
						applicationDir.toUri().toString()).get(YarnConfigKeys.FLINK_YARN_FILES));
			}
	...
	private ApplicationReport startAppMaster(
		...
		final List<String> userClassPaths = fileUploader.registerMultipleLocalResources(
				...
				userJarInclusion == YarnConfigOptions.UserJarInclusion.DISABLED ?
						ConfigConstants.DEFAULT_FLINK_USR_LIB_DIR : Path.CUR_DIR,LocalResourceType.FILE);
