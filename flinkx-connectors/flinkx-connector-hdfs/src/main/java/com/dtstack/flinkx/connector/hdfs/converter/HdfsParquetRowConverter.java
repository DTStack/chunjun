 ...
 * @author tudou
 */
public class HdfsParquetRowConverter extends AbstractRowConverter<RowData, RowData, Group, LogicalType> {
    ...
    protected IDeserializationConverter createInternalConverter(LogicalType type) {
        switch (type.getTypeRoot()) {
            ...
            case DATE:
                return (IDeserializationConverter<String, Integer>) val -> (int)DateUtil.getTimestampFromStr(val).toLocalDateTime().toLocalDate().toEpochDay();
            case FLOAT:
    ...
    protected ISerializationConverter<Group> createExternalConverter(LogicalType type) {
        switch (type.getTypeRoot()) {
            ...
            case TINYINT:
                return (rowData, index, group) -> group.add(columnNameList.get(index), rowData.getByte(index));
            case SMALLINT:
                return (rowData, index, group) -> group.add(columnNameList.get(index), rowData.getShort(index));
            case INTEGER:
