...
public class BinlogInputFormat extends BaseRichInputFormat {
    ...
    public void openInputFormat() throws IOException {
        ...
        LOG.info("binlog FilterBefore:{}, tableBefore: {}", binlogConf.getFilter(), binlogConf.getTable());
        ClassUtil.forName(BinlogUtil.DRIVER_NAME, getClass().getClassLoader());

        if (StringUtils.isNotEmpty(binlogConf.getCat())) {
            LOG.info("{}", categories);
            categories = Arrays.asList(binlogConf.getCat().toUpperCase().split(ConstantValue.COMMA_SYMBOL));
        }
        /*
    ...
    protected void openInternal(InputSplit inputSplit) {
        ...
        controller.setMasterInfo(new AuthenticationInfo(new InetSocketAddress(binlogConf.getHost(), binlogConf
                .getPort()), binlogConf.getUsername(), binlogConf.getPassword()));
    ...
    private void checkBinlogFile(String journalName) {
        if (StringUtils.isNotEmpty(journalName)) {
            if (!new BinlogJournalValidator(binlogConf.getHost(), binlogConf.getPort(), binlogConf.getUsername(), binlogConf
                    .getPassword()).check(journalName)) {
