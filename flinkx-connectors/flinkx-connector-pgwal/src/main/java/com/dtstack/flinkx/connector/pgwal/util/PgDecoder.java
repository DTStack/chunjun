...
public class PgDecoder {
    ...
    public static String unquoteIdentifierPart(String identifierPart) {
        ...
        if (quotingChar != null) {
            ...
            identifierPart = identifierPart.replace(quotingChar + quotingChar.toString(), quotingChar.toString());
        }
    ...
    private ChangeLog handleBeginMessage(ByteBuffer buffer) {
        ...
        LOG.trace("handleBeginMessage result = { lsn = {}, plus = {}, anInt = {}}", lsn, plus, transactionId);
    ...
    private ChangeLog handleCommitMessage(ByteBuffer buffer) {
        ...
        return new ChangeLog(PgMessageTypeEnum.COMMIT, transactionId, commitTimestamp.toEpochMilli());
    }
    ...
    private void handleRelationMessage(ByteBuffer buffer) throws SQLException {
        ...
        try (final PgConnection connection = RetryUtil.executeWithRetry(
            ...
            if (primaryKeyColumns == null || primaryKeyColumns.isEmpty()) {
                LOG.warn("Primary keys are not defined for table '{}', defaulting to unique indices", tableName);
                primaryKeyColumns = readTableUniqueIndices(databaseMetadata, null, schemaName, tableName);
            }
        }
...
//            if (optional == null) {
//                LOG.warn("Column '{}' optionality could not be determined, defaulting to true", columnName);
//                optional = true;
//            }
//
...
//        ChangeLog table = resolveRelationFromMetadata(new PgOutputRelationMetaData(relationId, schemaName, tableName, columnList, primaryKeyColumns));
...
//            ChangeLog existingTable = conf.getSchema().tableFor(new null, schemaName, tableName);
...
//            }
    ...
    List<String> readTableUniqueIndices(DatabaseMetaData metadata, String catalog, String schema, String table) throws SQLException {
    ...
    public List<String> readPrimaryKeyNames(DatabaseMetaData metadata, String catalog, String schema, String table) throws SQLException {
    ...
    private Map<String, Boolean> getTableColumnOptionalityFromDatabase(DatabaseMetaData databaseMetadata, String schemaName, String tableName) {
        ...
        try {
            ...
            LOG.warn("Failed to read column optionality metadata for '{}.{}'", schemaName, tableName);
