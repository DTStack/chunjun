...
package com.dtstack.flinkx.connector.hbase.util;
...
public class HBaseReadWriteHelper {
    ...
    public HBaseReadWriteHelper(HBaseTableSchema hbaseTableSchema) {
        ...
        this.rowKeyType =
                hbaseTableSchema.getRowKeyTypeInfo().map(HBaseTypeUtils::getTypeIndex).orElse(-1);
    ...
    public Get createGet(Object rowKey) {
        byte[] rowkey = HBaseTypeUtils.serializeFromObject(rowKey, rowKeyType, charset);
        ...
        for (int i = 0; i < fieldLength; i++) {
            if (rowKeyIndex == i) {
                ...
                for (int q = 0; q < this.qualifiers[f].length; q++) {
                    ...
                    if (value != null) {
                        familyRow.setField(
                                q, HBaseTypeUtils.deserializeToObject(value, typeIdx, charset));
                    } else {
    ...
    public Put createPutMutation(Row row) {
        ...
        byte[] rowkey =
                HBaseTypeUtils.serializeFromObject(row.getField(rowKeyIndex), rowKeyType, charset);
        // upsert
        ...
        for (int i = 0; i < fieldLength; i++) {
            if (i != rowKeyIndex) {
                ...
                for (int q = 0; q < this.qualifiers[f].length; q++) {
                    ...
                    // read value
                    byte[] value =
                            HBaseTypeUtils.serializeFromObject(
                                    familyRow.getField(q), typeIdx, charset);
