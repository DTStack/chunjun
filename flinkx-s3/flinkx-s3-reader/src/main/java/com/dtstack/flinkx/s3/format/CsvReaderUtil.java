...
public class CsvReaderUtil {
	...
	public CsvReaderUtil(String fileName, char delimiter, Charset charset, Long nextOffset)
			throws FileNotFoundException {
		if (fileName == null) {
	...
	public CsvReaderUtil(Reader inputStream, char delimiter) {
		this(inputStream,delimiter,0L);
	}

	public CsvReaderUtil(Reader inputStream, char delimiter, Long nextOffset) {
		if (inputStream == null) {
			throw new IllegalArgumentException(
	...
	public CsvReaderUtil(InputStream inputStream, char delimiter, Charset charset, Long nextOffset) {
		this(new InputStreamReader(inputStream, charset), delimiter,nextOffset);
	}

	public CsvReaderUtil(InputStream inputStream, char delimiter, Charset charset) {
		this(new InputStreamReader(inputStream, charset), delimiter,0L);
	}
				...
				if (rawBuffer.Position == 0) {
					...
					rawRecord = new String(rawBuffer.Buffer, 0,
							...
							+ new String(dataBuffer.Buffer,
									dataBuffer.LineStart, dataBuffer.Position
											- dataBuffer.LineStart - 1);
					nextOffset = nextOffset + rawBuffer.Position+ dataBuffer.Position - dataBuffer.LineStart;
				}
			} else {
