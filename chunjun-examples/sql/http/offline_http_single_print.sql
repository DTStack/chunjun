...
(
    ...
    createTime                varchar
      ...
      ,'returned-data-type'='single'                 -- 返回数据类型：single：单条数据；array：数组数据
      ,'json-path'='$'                        -- json路径：jsonpath用于解析指定层的json数据
                                                   -- 以下4个参数要同时存在：
      ,'page-param-name'=''                          -- 多次请求参数1：分页参数名：例如：pageNum
      ,'start-index'='1'                             -- 多次请求参数2：开始的位置
      ,'end-index'='1'                               -- 多次请求参数3：结束的位置
