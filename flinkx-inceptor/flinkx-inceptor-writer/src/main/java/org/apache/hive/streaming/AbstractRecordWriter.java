...
//        String outFormatName = this.table.getSd().getOutputFormat();
        ...
        try {
            ...
            String shadePrefix = conf.get("hive.classloader.shade.prefix");
...
//    this.autoFlush = conf.getBoolean("hive.streaming.auto.flush.enabled",true);
//    this.memoryUsageThreshold = conf.getFloat("hive.heap.memory.monitor.usage.threshold",0.7f);
//    this.ingestSizeThreshold = toSizeBytes(conf.get("hive.streaming.auto.flush.check.interval.size","100mb"));
...
//  }
    ...
    protected int getBucket(Object row) {
        ...
        int bucketingVersion = getBucketingVersion(
                table.getParameters().get("bucketing_version"));
        return getBucketNumber(getBucketHashCodeOld(bucketFields, bucketObjInspectors), totalBuckets);
    }
    ...
    protected RecordUpdater createRecordUpdater(final Path partitionPath, int bucketId, Long minWriteId,
        ...
        return acidOutputFormat.getRecordUpdater(partitionPath,
                new AcidOutputFormat.Options(conf)
                        ...
                        .maximumTransactionId(maxWriteID)
                        .minimumTransactionId(minWriteId)
        );
    }
